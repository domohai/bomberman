...
public class PathFinder {
    //Using of Dijkstra is not optimal (slower than BFS) when all path have the same length = 1
...
//    }
    ...
    public static int pathFinderBFS(int si, int sj, char[][] map, boolean ghost) {
        int n = map.length;
        int m = map[0].length;
        //find the player pos
        int ti = si, tj = sj;
        ...
        while (!pq.isEmpty()) {
            Waypoint p = pq.poll();
            if (p.d > f[p.i][p.j]) continue;
            //!!
            if (p.i == ti && p.j == tj) break;
            if (p.i - 1 >= 0 && (map[p.i - 1][p.j] == ' ' || (ghost && map[p.i - 1][p.j] != '#')) && f[p.i - 1][p.j] > p.d + 1) {
                pq.add(new Waypoint(p.i - 1, p.j, p.d + 1));
                f[p.i - 1][p.j] = p.d + 1;
                trace[p.i - 1][p.j] = 1;
            }
            if (p.i + 1 < n && (map[p.i + 1][p.j] == ' ' || (ghost && map[p.i + 1][p.j] != '#')) && f[p.i + 1][p.j] > p.d + 1) {
                pq.add(new Waypoint(p.i + 1, p.j, p.d + 1));
                f[p.i + 1][p.j] = p.d + 1;
                trace[p.i + 1][p.j] = 2;
            }
            if (p.j - 1 >= 0 && (map[p.i][p.j - 1] == ' ' || (ghost && map[p.i][p.j - 1] != '#')) && f[p.i][p.j - 1] > p.d + 1) {
                pq.add(new Waypoint(p.i, p.j - 1, p.d + 1));
                f[p.i][p.j - 1] = p.d + 1;
                trace[p.i][p.j - 1] = 3;
            }
            if (p.j + 1 < m && (map[p.i][p.j + 1] == ' ' || (ghost && map[p.i][p.j + 1] != '#')) && f[p.i][p.j + 1] > p.d + 1) {
                pq.add(new Waypoint(p.i, p.j + 1, p.d + 1));
                f[p.i][p.j + 1] = p.d + 1;
                trace[p.i][p.j + 1] = 4;
            }
